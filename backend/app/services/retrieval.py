...
# async def hybrid_search(query: str, org_id: str, top_k: int = 20) -> list:
#     # 1. Vector search
...
#     corpus = [c.payload["text"] for c in all_chunks]
...
#         reverse=True
#     )[:top_k]
#     bm25_results = [all_chunks[i] for i in bm25_top_idx]
...
#     return [all_results[pid] for pid in sorted_ids[:top_k] if pid in all_results]
